...
export default function HintBar({ secondsLeft, answerPattern, revealedHints }) {
  ...
  // Walk through each word and track the running flat character index so we
  // can match revealedHints (which use the full-string charIndex) back to
  // the correct blank slot.
  // Word i starts at offset = sum of (wordLengths[0..i-1]) + i  (the +i
  // accounts for the single space between each word).
  let flatOffset = 0
    ...
    <div className="hint-bar">
      <div className="hint-blanks">
        {(answerPattern || []).map((wordLength, wordIndex) => {
          ...
          flatOffset += wordLength + 1 // +1 for the space after this word
      ...
      <div className="hint-timer">{secondsLeft}s</div>
    </div>
  )
}